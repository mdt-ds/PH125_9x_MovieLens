...
In the sparse user-item interaction matrix, the predicted rating user u will give item i is computed as: $$r_{ui}=\sum_{f=0}^{nfactors}h_{u,f} \cdot w_{f,i}$$ 
...
## Proposed model
The proposed model should take into considaration the global mean $\mu$ effect, the group level effects for movie, user and movie year and will add the latent factor effect.
...
```
The model could be considered useful if the RMSE metric reach a value below the one reached with the null model based only on the overall mean `r mean(edx$rating)`.
```{r rmse_null, cache=TRUE}
rmse_null <- RMSE(validation$rating, mean(edx$rating))
print(rmse_null)
```
The model will be constructed in 3 steps first modeling effects, then regularizing the model and finally adding the latent factor effect provided by the rating matrix factorization.
...
```
The best $\lambda$ is equal to `r lambda_best` and it can be so used to produce predictione and evaluating the model.
...
softImpute implements an alternate least square algorithm that allow the factorization also in presence of a matrix with high rate of sparsity.
...
```
The incomplete matrix object is now ready to be scaled by row (user) and column (movie) and factorized in 50 latent factor with 'als' algorithm. Note that lambda regularizing parameter has to be chosen so that the rank is 50 (It has been done through a parameter search but not reported in script for execution time constraint).
...
rating_fits$d
```
Note that lambda regularizing parameter has to be chosen so that the rank is 50.
It is now possible to recontruct the matrix for validaion observation.
...
The final RMSE is below the null model rmse , about`r round(rmse_null, 2)`, by `r round((1-(rmse_2/rmse_null)), 2) * 100`%  and also below the threshold set by HarvardX course Team to achieve 25 points.
Every step in model building make the prediction better and better.
...
Furthermore make me recall my former study in linear algebra as a student of electrical engineer years ago and understanding its application in the context of data science.
...
Furthermore we can explore the other algorithm for matrix completion of matrix provided by the softImpute package.
As far as script performance is concerned for sure there is room for improvement in recoding for loops: options are parallelizing for loop or use Rcpp function to speed up the execution.
...
R script and rmarkdown file are available for review on public github repository: https://github.com/mdt-ds/PH125_9x_MovieLens .

## Computation
Computation capacity is a critical point: enough RAM shall be provided in order to perform the creation of the rating matrix.
...
For this reason I have commented out this part on the script in order to allow running the script without error if memory of your computer is not enough. The script commented out runs in about 10 minutes on my laptop.
...
***
\centering
Report under  ![](images/cc-by-nc-sa.png)  license
